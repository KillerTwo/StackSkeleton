...
func InitWebRouter() *gin.Engine {
	...
	{
		routers.RouterGroupApp.CaptchaRouter.InitCaptchaRouter(verifyCode)
	}
	//  创建一个后端接口路由组
	backend := router.Group("/admin/")
	{
		...
		routers.RouterGroupApp.WebSocketRouter.InitWebSocketRouter(backend)

		//  【不需要token】中间件验证的路由  用户注册、登录
		noAuth := backend.Group("users/")
		{
			// 不需要验证码即可登陆
			routers.RouterGroupApp.UserRouter.InitNoAuthUserRouter(noAuth)
		...
		menuRouter := backend.Group("/menus")
		menuRouter.Use(authorization.CheckTokenAuth())
		{
			routers.RouterGroupApp.MenuRouter.InitMenuRouter(menuRouter)
		}
		...
		// 【需要token】中间件验证的路由
		auth := backend.Group("users/")
		auth.Use(authorization.CheckTokenAuth())
		{
			// 用户组路由
			// users := backend.Group("users/")
			/*{
				// 查询 ，这里的验证器直接从容器获取，是因为程序启动时，将验证器注册在了容器，具体代码位置：App\Http\Validator\Web\Users\xxx
				routers.RouterGroupApp.UserRouter.InitUserRouter(backend)
			}*/
			routers.RouterGroupApp.UserRouter.InitUserRouter(auth)
			//文件上传公共路由
			uploadFiles := backend.Group("upload/")
			{
				// uploadFiles.POST("files", validatorFactory.Create(consts.ValidatorPrefix+"UploadFiles"))
				routers.RouterGroupApp.UploadRouter.InitUploadRouter(uploadFiles)
			}
		}
	}
	return router
