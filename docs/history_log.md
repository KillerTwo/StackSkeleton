...
#### V 1.4.08  2020-11-24
* 功能完善 ：  
  1.移除 `tb_auth_users` model中的一处调试信息.

#### V 1.4.07  2020-11-19
* 功能完善 ：  
  1.简化v1.3版本中遗留的 `tb_auth_users` 查询代码.
...
#### V 1.4.03  2020-11-01
* bug修复：  
  1.由于tb_auth_users 表字段 token 在新版中在独立的表处理，相关查询sql没有及时移除该字段导致一处bug发生.
* 功能完善：  
...
>   3.`token`部分与`tb_auth_users`逻辑交互代码更新，主要有登录生成token、刷新`token`、用户更改密码，重置相关的`token`使之失效，用户删除数据，同步删除相关的token表数据。     
...
>   7.精简代码，基本的业务操作只保留`tb_auth_users`表的完整操作示例代码。         
